...
class ControllerMember extends Controller
{
	public function initialize(){
        ...
        foreach ($navLeft as $key => $value) {
            if($user&&$users->is_broker){
                foreach ($value["child"] as $k => $v) {
                   if($k==2||$k==4||$k==31||$k==23){
        ...
        //$pes=$this->selectByUserFarms(M\UserInfo::USER_TYPE_PE);

        // if($pe){
        // $this->view->user_pe = $pe;
        // }else{
        // $this->view->user_pe =0;
        // $this->view->user_vs =0;
        // }
        ...
        $this->view->userinfosid = md5(session_id());
    ...
    public function checkUserInfo() {
        ...
        if(!$uid) return 1;
        $info = M\Users::findFirstByid($uid);
        if(!$info) return 1;
	...
	public function checkLoginInfo() {
        ...
        $address = M\AreasFull::checkarea($users->areas);
