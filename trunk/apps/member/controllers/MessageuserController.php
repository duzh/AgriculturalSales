...
class MessageuserController extends ControllerMember
{
    
    public function listAction() 
    {   
        if(!isset($this->session->time) || empty($this->session->time) ){
        ...
        $count=M\Message::GetMessageUnreadCount($user_id);
        //var_dump($user_id);die;
        foreach ($data as $key => $value) 
        {
            $message = M\Message::findFirst("status = 1 and msg_id = " . $value['msg_id']);
            ...
            else
            {
                ...
                else
                {
                    ...
                    if ($message->type == 1) 
                    {
                        $data[$key]['desc'] = "<p>供应信息推荐：{$message->buyer_name}推出了新的供应商品，点击<a href='/member/messageuser/info/{$message->msg_id}'>查看详情</a></p>";
                        $data[$key]['name'] = "采购推荐";
                    }
                    ...
                    else
                    {
                        ...
                        elseif ($message->link_type == 1) 
                        {
                            ...
                            if ($order->state == 1) 
                            {
                                $data[$key]['desc'] = "<p>" . $message->comment . ", <a href='/member/orderssell/index'>点击查看</a></p>";
                                $data[$key]['name'] = "系统消息";
                            }
                            else
                            {
                                $data[$key]['desc'] = "<p>" . $message->comment . ", <a href='/member/orderssell/info/{$message->order_id}'>点击查看</a></p>";
                                $data[$key]['name'] = "系统消息";
                            }
                            $data[$key]['name'] = "系统消息";
                        }
                        ...
                        if ($message->link_type == 3) 
                        {
                            ...
                            if ($order->state == 1) 
                            {
                                $data[$key]['desc'] = "<p>" . $message->comment . ", <a href='/member/ordersbuy/index'>点击查看</a></p>";
                                $data[$key]['name'] = "系统消息";
                            }
                            else
                            {
                                $data[$key]['desc'] = "<p>" . $message->comment . ", <a href='/member/ordersbuy/info/{$message->order_id}'>点击查看</a></p>";
                                $data[$key]['name'] = "系统消息";
                            }
                            $data[$key]['name'] = "系统消息";
                        }
        ...
        $datas['total_pages'] = ceil($total / $page_size);
        $this->view->current = $p;
        $this->view->data = $datas;
        $this->view->title = '个人中心-消息管理';
    }
    ...
    public function infoAction($msg_id = '') 
    {
        ...
        $message = M\Message::findFirst("status = 1 and msg_id = " . $msg_id);
        ...
        $messageusers = M\MessageUsers::findFirst("user_id = {$user_id} and msg_id = {$msg_id}");
