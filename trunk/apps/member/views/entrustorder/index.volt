        ...
        <div class="w1185 mtauto clearfix">
            ...
            <div class="center-right f-fr">
                ...
                <div class="business-order">
                    ...
                    <div class="title f-oh">我买入的订单</div>

                    <!-- 买入 -->
                    <div class="order-tab" style="display:block;">
                        ...
                        <table cellpadding="0" cellspacing="0" width="903" class="list">
                            ...
                            <tr height="32" class="serial">
                                <td colspan="4">
                                    <span>订单号：<i>{{ order.order_sn }}</i></span>
                            ...
                            <tr height="122">
                                <td align="center">
                                    {% if order.sellid!=0 %}
                                    <img src="<?php echo $sell=Mdg\Models\Sell::getSellThumb($order->sellid); ?>" height="100" width="90">
                                    {% else %}
                                    <img src="http://static.ync365.com/mdg/images/detial_b_img.jpg" height="100" width="90">
                                    {% endif %}
                                </td>
                                <td>
                                    ...
                                    <div class="info clearfix">
                                        <font>单品价格：</font>
                                        <div class="info-box">
                                            {% if order.state == 2 %}<?php $orderprice=Mdg\Models\Sell::getprice($order->sellid); if($orderprice){ echo $orderprice;}else{ echo $order->price;}?> {% else %}{{ order.price }}{% endif %}元/{{ goods_unit[order.goods_unit] }}
                                        </div>
                                    </div>
                                    <div class="info clearfix">
                                        <font>购买数量：</font>
                                        <div class="info-box">
                                            {{ order.quantity }}{{ goods_unit[order.goods_unit] }}
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td align="center">
                                    ...
                                    <a  href="/member/ordersbuy/payorder/{{ order.id }}"   onclick="showDistplay({{ order.id}})" target="_blank" >去付款</a>

                                    {% endif %}
                                    {% if (order.state == 5) %}
                                     <a  href="javascript:finishOrder({{ order.id }})">确认收货</a>
                                    {% endif %}

                                    {% if (order.state == 6) and !order.activity_id %}
                                    <?php $goods_id=$order->purid?$order->purid:$order->sellid; $comments=Mdg\Models\GoodsComments::WhetherComments($goods_id,$user_id);if($comments=='no'){
                                     echo "<a class='f-db pingfen mt10' href='javascript:;' onclick='pingfen($order->purid, $order->sellid)'>去评分</a>";}else{echo '已评论';}?>
                        ...
                        <!-- 分页 -->
                        <div class="center-page pr20 f-tar mt20 mb20">
                            {{ pages }}
                        </div>
...
function showDistplay(oid) {
   ...
   $('#show').show();
    ...
    $(function(){
        ...
        $('.business-order .pingfen').click(function(){
	...
	function subcomments(){
		...
		if (msg != '')
		{
			alert(msg);
				...
				data:{'purid':purid,'sellid':sellid,'decribe_score':decribe_score,'comment':comment,'anonym_flag':anonym_flag},
				dataType:"json",
				success:function(data){		
						if (data.status=='1000')
						{
