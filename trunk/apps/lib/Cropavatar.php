...
private function setFile($file,$path) {
  ...
  if ($errorCode === UPLOAD_ERR_OK) {
    ...
    if ($type) {
      ...
      if ($type == "IMAGETYPE_GIF" || $type == "IMAGETYPE_JPEG" || $type == "IMAGETYPE_PNG") {
        ...
        //   unlink($src);
        // }
         $result=File::move_file($file, PUBLIC_PATH.$path, true);
        // $result = move_uploaded_file($file['tmp_name'], $src);
          
        if ($result) {
          $this -> src = $result["path"];
          $this -> type = $type;
...
private function setDst() {
   ...
   if (!file_exists(PUBLIC_PATH.$path)) File::make_dir(PUBLIC_PATH.$path);   
    $ext = pathinfo($this -> src, PATHINFO_EXTENSION);
    
    $ext=strtoupper($ext);

   $this -> dst = PUBLIC_PATH.$path.File::random(12 , 1).'.'.$ext;
...
public function getResult() {
  ...
  $dst = str_replace(PUBLIC_PATH,'',$this -> dst);
  return !empty($this -> data) ? $dst : $this -> src;
}
