...
class Validator
{
     ...
     *
     * @code php
     * if (!Validator::validate($value, 'max', 5))
     * {
     *     echo 'value 不能大于 5';
     * }
     * @endcode
     ...
     *
     * @code php
     * Validator::validate($value, 'between', 1, 5);
     * @endcode
     *
     ...
     * @code php
     * // 使用某个类的静态方法做验证方法
     * Validator::validate($value, array('MyClass', 'myMethod'), $args);
     *
     * // 功能同上
     * Validator::validate($value, 'MyClass::myMethod', $args);
     *
     * // 使用某个对象的方法做验证方法
     * Validator::validate($value, array($my_obj, 'myMethod), $args);
     * @endcode
     *
    ...
    {
        return mb_strlen($value,self::CHARTYPE) >= $len;
    }
    ...
    {
        return @(preg_match('/^[\u4e00-\u9fa5]+|\w+$/', $value) == 0);
    }
    /**
     * 是否是控制字符
     *
     ...
     * @return boolean
     */
    static function validate_is_mobile($value)
    {
        //return preg_match('/^[a-z0-9]+[._\-\+]*@([a-z0-9]+[-a-z0-9]*\.)+[a-z0-9]+$/i', $value);
        return preg_match('/^1[0-9]{10}$/', $value);
        // return preg_match('/^13[0-9]{9}|15[012356789][0-9]{8}|18[0256789][0-9]{8}|147[0-9]{8}$/', $value);
    }
    ...
    static function validate_is_http($value)
    {
