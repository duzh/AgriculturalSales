...
class ServicestationController extends ControllerBase
{
    /**
     * 服务站列表
     */
    ...
    public function indexAction() 
    {
        ...
        $shop_no = $this->request->get('shop_no', 'string', '');
        ...
        $cond[] = $conds;
        $cond['columns'] = ' province_id,city_id,county_id,town_id,village_id,contact_man, contact_phone, shop_id, add_time, business_type, user_type, shop_type, shop_status, user_id, shop_name, shop_no ';
    ...
    public function getAction($id = 0) 
    {
        ...
        if (!$id) $this->response->redirect("servicestation/index")->sendHeaders();
        $data = M\Shop::getShopInfo($id);
        $main_product = '';
        $name = array();
        $Viewareas = array();
        if (!$data) $this->response->redirect("servicestation/index")->sendHeaders();
        /**
         * 获取地区
         * @var [type]
         */
         ...
         * @var [type]
         */
        $areas = M\ServiceArea::find(" service_id = '{$data->shop_id}'")->toArray(); 
        ...
        M\TmpFile::clearall($sid, M\TmpFile::SERVICEPIC);
        
        //查询实地照片
        $pic = M\ServicePic::find(" service_id = '{$data->shop_id}'")->toArray();
        $count = 5- count($pic);
        //查询所在地区
        $this->view->countPic =$count;  
        $this->view->pic = $pic;
        $this->view->sid = $sid;
        ...
        $this->view->address = join('',$areasName);
    ...
    public function savepicAction () {
        ...
        $type =  M\TmpFile::SERVICEPIC;
        $service_demo = $this->request->getPost('service_demo','string','');
        $time = time();
        $data = M\TmpFile::find(" sid = '{$sid}' AND type = '{$type}'");
        if($data) {
            foreach ($data as $key => $val) {
                $ServicePic = new M\ServicePic();
                $ServicePic->service_id = $shopid;
                $ServicePic->pic_path = $val->file_path;
                $ServicePic->add_time = $time;
                $ServicePic->last_update_time = $time;    
                $ServicePic->save();
            }
        }
        $credit = M\ShopCredit::findFirst(" shop_id = '{$shopid}'");
        if($credit) {
            //添加 服务站介绍
            $credit->service_desc = $service_demo;
            $credit->save();
        }
        
        
    $this->response->redirect("servicestation/index")->sendHeaders();
     ...
     * @return [type] [description]
     */
    public function removepicimgAction() {
        $id = $this->request->get('id', 'int', 0 );
        $cond[] = " id ='{$id}'";
        $data = M\ServicePic::delImages($cond);
    ...
    public function editAction($id = 0) 
    {
        ...
        $shopgoods = M\ShopCoods::find("shop_id=$shop->shop_id")->toarray();
        $sid = md5(session_id());
    ...
    public function shopauditAction() 
    {
        ...
        if (!$this->request->getPost()) $this->response->redirect("servicestation/index")->sendHeaders();
        $flag = false;
        ...
        Func::adminlog("店铺审核通过：{$data->shop_name}",$this->session->adminuser['id']);
        $this->response->redirect("servicestation/index")->sendHeaders();
    }
    /**
     * 店铺审核不通过
    ...
    public function shopunauditunAction() 
    {
        ...
        Func::adminlog("店铺审核未通过：{$data->shop_name}",$this->session->adminuser['id']);
        $this->response->redirect("servicestation/index")->sendHeaders();
    }
