...
<link rel="stylesheet" type="text/css" href="{{ constant('STATIC_URL') }}mdg/manage/css/style.css" />
  ...
  <div class="main_right">
    ...
    <div class="chaxun">
      {{ form("/goodscomments/index", "method":"get", "autocomplete" : "off") }}
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        ...
        <tr>
          <td  width="15%" height="35" align="right">被评论商品：</td>
          <td  width="35%" height="35" align="left">{{ text_field("goods_name") }}</td>
    ...
    <div class="neirong" id="tb">
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        ...
        <tr align="center">
          ...
          <td width="10%">{{item.goods_name}}</td>
          <td width="15%" class="txt">
				 描述相符：{{item.decribe_score}}星
          </td>
          <td width="20%" class="txt">{{item.comment}}</td>
          <td width="10%"><?php echo isset(Mdg\Models\GoodsComments::$_is_check[$item->is_check]) ? Mdg\Models\GoodsComments::$_is_check[$item->is_check] : ''; ?></td>
          <td width="8%">

            {% if item.is_check == 0 %}
            <a href="/manage/GoodsComments/is_checksave/{{item.id}}/1" >审核通过</a> 
            <a href="/manage/GoodsComments/is_checksave/{{item.id}}/2">审核未通过</a> 
          </td>
            {% endif %}
