...
<div class="main_right">
    ...
    **  代码从这开始
    ...
    <div class="bt2">HC会员</div>

    <div class="tab2">
        <ul id="test2_li_now_">
            <a href='/manage/corporate/hc?show=1'><li <?php if(empty($_GET['show']) || $_GET['show']==1){?>class="now"<?php }?>>会员列表</li></a>
            <a href='/manage/corporate/hc?show=2'><li <?php if(!empty($_GET['show']) && $_GET['show']==2){?> class="now"<?php }?>>待审核</li></a>
        </ul>
    </div>
<?php if(empty($_GET['show']) || $_GET['show']==1){?>
    <div id="test2_1" class="tablist block">
        <!-- 查询 -->
        {{ form("corporate/hc", "method":"get", "autocomplete" : "off") }}
        ...
        <div class="neirong">
            ...
            {% for key, item in users['items'] %}
                ...
                <tr align="center">
                  ...
                  <td >
                    <a href="/manage/corporate/hcpersonalinfo/{{ item['user_id']}}/2/hc/{{item['credit_id']}}">查看</a>
              
                    <a href="/manage/corporate/hcedit/{{ item['user_id']}}/2/hc/{{item['credit_id']}}">编辑</a>
              ...
              {% for key, item in userinfo['items'] %}
                <tr align="center">
                  ...
                  <td >
                    ...
                    <a href="/manage/corporate/hcedit/{{ item['user_id']}}/2/hc/{{item['credit_id']}}">编辑</a>
                    {% if item and item['status']==1 %}
                      <a href="/manage/corporate/get/{{ item['user_id']}}/2/hc/{{item['credit_id']}}">取消认证</a>
                    {% endif %}
...
function setreject(){
    ...
    if($('#zjuser_id').val() && $('#zjcredit_id').val()){
        window.location='/manage/corporate/get/'+$('#zjuser_id').val()+'/2/hc/'+$('#zjcredit_id').val()+'?reject='+$("input[name='reject']").val();
    }
    else{
        alert('参数错误');
    }
