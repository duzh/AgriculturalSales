...
<div class="main_right">
    ...
    **  代码从这开始
    ...
    <div class="bt2">PE会员</div>

    <div class="tab2">
        <ul id="test2_li_now_">
            <a href='/manage/corporate/pe?show=1'><li <?php if(empty($_GET['show']) || $_GET['show']==1){?>class="now"<?php }?>>会员列表</li></a>
            <a href='/manage/corporate/pe?show=2'><li <?php if(!empty($_GET['show']) && $_GET['show']==2){?> class="now"<?php }?>>待审核</li></a>
        </ul>
    </div>
<?php if(empty($_GET['show']) || $_GET['show']==1){?>
    <div id="test2_1" class="tablist block">
        <!-- 查询 -->
        {{ form("corporate/pe", "method":"get", "autocomplete" : "off") }}
        <div class="chaxun vip-list">
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
                ...
                <tr height="40">
                    <th width="15%">主营业务</th>
                    <td width="35%">
                      <select name="category_namea" class='selectAreasa' id="">
                          <option value="">请选择</option>
                      </select>
                      <select name="category_nameb" class='selectAreasa' id="">
                          <option value="">请选择</option>
                      </select>
                    </td>
                    <th width="15%"></th>
        ...
        <div class="neirong">
            ...
            {% for key, item in users['items'] %}
                ...
                <tr align="center">
                  ...
                  <td >{% if item and item['type']==1%}{{item['company_name']}}{% else %}{% endif %}
                       {% if item and item['type']==0%}{{item['name']}}{% else %}{% endif %}</td>
                  <td >{% if item and item['type']==1 %}企业{% elseif item and item['type']==0 %}个人{% endif %}</td>
                  ...
                  <td >{% if item['farm'] %}{% for v in item['farm']%}{{v['category_name']}},{% endfor %}{% endif %}</td>
                  <td >{{ date('Y-m-d H:i:s', item['add_time'] ) }}</td>
                  <td >{% if item %}<?=Mdg\Models\UserInfo::$_status[$item['status']]?>{% else %}-{% endif %}</td>
                  <td >
                    <a href="/manage/corporate/hcpersonalinfo/{{ item['user_id']}}/16/pe/{{item['credit_id']}}">查看</a>
                    <!-- <a href="/manage/corporate/hcedit/{{ item['user_id']}}/16/pe">编辑</a> -->
                    <a href="/manage/corporate/peedit/{{ item['user_id']}}/16/pe/{{item['credit_id']}}">编辑</a>
                    {% if item and item['status']==1 %}
                      <a href="javascript:ShowDiv('MyDiv1','fade1','{{ item['user_id']}}','{{item['credit_id']}}');">取消认证</a>
                      {% endif %}
            ...
            {% for key, item in userinfo['items'] %}
                ...
                <tr align="center">
                  ...
                  <td >
                    ...
                    <a href="/manage/corporate/peedit/{{ item['user_id']}}/16/pe/{{item['credit_id']}}">编辑</a>
                    {% if item and item['status']==1 %}
                    <a href="javascript:if(confirm('确认取消认证吗?'))window.location='/manage/corporate/get/{{ item['user_id']}}/16/pe/{{item['credit_id']}}'">取消认证</a>
                    {% endif %}
...
<script type="text/javascript">
    ...
    //弹出隐藏层
    function ShowDiv(show_div,bg_div,user_id,credit_id){
...
$(".selectAreasa").ld({ajaxOptions : {"url" : "/ajax/getcate"},
    defaultParentId : 0,
    {% if start_areasa %}
    texts:[{{ start_areasa}}],
    {% endif %}
