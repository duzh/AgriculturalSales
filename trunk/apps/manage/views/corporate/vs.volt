...
<div class="main_right">
    ...
    **  代码从这开始
    ...
    <div class="bt2">VS会员</div>

    <div class="tab2">
        <ul id="test2_li_now_">
            <a href='/manage/corporate/vs?show=1'><li <?php if(empty($_GET['show']) || $_GET['show']==1){?>class="now"<?php }?>>会员列表</li></a>
            <a href='/manage/corporate/vs?show=2'><li <?php if(!empty($_GET['show']) && $_GET['show']==2){?> class="now"<?php }?>>待审核</li></a>
        </ul>
    </div>
<?php if(empty($_GET['show']) || $_GET['show']==1){?>
    <div id="test2_1" class="tablist block">
        <!-- 查询 -->
        {{ form("corporate/vs", "method":"get", "autocomplete" : "off") }}
        ...
        <div class="neirong">
            ...
            {% for key, item in users['items'] %}
                ...
                <tr align="center">
                  ...
                  <td ><?php if (isset($item['add_time']) && $item['add_time']){echo date("Y-m-d H:i:s",$item['add_time']);}?></td>
                  <td >{% if item %}<?=Mdg\Models\UserInfo::$_status[$item['status']]?>{% else %}-{% endif %}</td>
                  <td >
                    <a href="/manage/corporate/hcpersonalinfo/{{ item['user_id']}}/4/vs/{{item['credit_id']}}">查看</a>
                  
                    <a href="/manage/corporate/vsedit/{{ item['user_id']}}/4/vs/{{item['credit_id']}}">编辑</a>
            ...
            {% for key, item in userinfo['items'] %}
                ...
                <tr align="center">
                  ...
                  <td >
                    ...
                    {% if item and item['status']==1 %}
                      <a href="javascript:if(confirm('确认取消认证吗?'))window.location='/manage/corporate/get/{{ item['user_id']}}/4/vs/{{item['credit_id']}}'">取消认证</a>
                    {% endif %}
...
<script type="text/javascript">
    function setreject(){
        ...
        if($('#zjuser_id').val() && $('#zjcredit_id').val()){
            window.location='/manage/corporate/get/'+$('#zjuser_id').val()+'/4/vs/'+$('#zjcredit_id').val()+'?reject='+$("input[name='reject']").val();
        }
        else{
            alert('参数错误');
        }
