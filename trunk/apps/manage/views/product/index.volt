...
<div class="main">
    <div class="main_right">
        <div class="bt2">案例列表</div>
        {{ form("/product/index", "method":"post", "autocomplete" : "off") }}
        <div class="chaxun">
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
            ...
            {% for article in data.items %}
                <tr  align="center">
                    <td><?php echo $i++ ;?></td>
                    <td>{{ article.title }}</td>
                    <td><?php echo Mdg\Models\Product::returncategory($article->cid)?></td>
                    <td>{{ isshow[article.is_show] }}</td>
                    <td>{{ date('Y-m-d H:i:s', article.addtime) }}</td>
                    <td>{{ article.count }}</td>
                    <td>{{ link_to("http://mdg.ync365.com/product/info?p="~article.id, "查看",'target' : '_blank') }}
                        {{ link_to("product/edit/"~article.id, "修改") }}
...
<script type="text/javascript">
 ...
 {  
    if(confirm("您确定要删除吗?"))
    location.href='/manage/product/delete/'+id;
 }
</script>
</html>
