...
<!-- 头部结束 -->
<div class="w960 ur_here " style="margin-top:40px;" ><a href="/index">首页</a>&nbsp;>&nbsp;<a href="/product/category?c={{catid}}">{{catname}}</a>&nbsp;>&nbsp;产品案例标题</div>
    ...
    <div class="gl">
            ...
            <span class="f-fl">上一篇：<em>
              <a href="/product/info?p={{ prev.id }}">{{ prev.title }}</a></em></span>
            {% endif %}
            {% if (next) %}
            <span class="f-fr">下一篇：<a href="/product/info?p={{ next.id }}">{{ next.title }}</a></span>
            {% endif %}
    </div>
</div>
<!-- 底部开始 -->
